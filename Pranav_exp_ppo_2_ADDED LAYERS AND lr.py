...
fieldnames2 = ['epsodic reward' , 'total_positive_reward' , 'total_negative_reward' ,'closest_dist' ,'max_height' ,'end_distance' ,'max_distance' , 'up_actions(100)' , 'down_actions(0)', 'highest_x' , 'highest_y' , 'highest_pitch' , 'highest_roll' , 'epsodic reward_m' , 'episodic reward_d','episodic reward_yaw' ,'episodic reward_pr','wrong action' ]
m1 = 0
m2 = 0
m3 = 0
m4 = 0
...
global roll, pitch, yaw , lidar_distance ,velocity , global_x , global_y  , episode , total_reward_m, total_reward_d ,total_reward_yaw , total_reward_pr , closest_dist , distance_goal , name_data , total_positive_reward , total_negative_reward  , end_distance , max_distance , episode_number
episode_number = 0
...
class Environment():
    ...
    def step(self,action,state ,w_a):
        ...
        #print(action , m1,m2,m3,m4)

        #print(lidar_distance,m1,m2,m3,m4)
    ...
    def get_reward(self , state, action , wrong_action):        
        global m1 , m2 , m3 , m4 ,lidar_distance ,global_y , global_x ,roll, pitch, yaw, episode , total_reward_m, total_reward_d ,total_reward_yaw , total_reward_pr , closest_dist , distance_goal , name_data , total_positive_reward , total_negative_reward , end_distance , max_distance 
        ...
        if lidar_distance < 4 and action == 0:
            reward_d = 0.01        
        if lidar_distance > 4 and action == 1:
            reward_m = 10
...
class PPO():
    def __init__(self):
        ...
        self.act_dim = 6 #self.env.action_space.shape
        ...
        self.actor_optim = Adam(self.actor.parameters(), lr= 1e-4 ,weight_decay=1e-5)
        self.critic_optim = Adam(self.critic.parameters(), lr= 1e-3 , weight_decay=1e-5)
    ...
    def rollout(self):
        ...
        while t < self.timesteps_per_batch:
            ...
            with open(name,'a') as csv_file_2:
                csv_writer_2 = csv.DictWriter(csv_file_2 , fieldnames = fieldnames2)
                information2 = {"epsodic reward" : sum(ep_rews) ,"total_positive_reward" : total_positive_reward, "total_negative_reward" : total_negative_reward, "closest_dist" : closest_dist, "max_height" : highest_point,"end_distance": end_distance,"max_distance": max_distance, "up_actions(100)":up_actions , "down_actions(0)":down_actions, "highest_x" : highest_x, "highest_y" : highest_y , "highest_pitch" : highest_pitch, "highest_roll" : highest_roll,"epsodic reward_m" : total_reward_m, "episodic reward_d" : total_reward_d, "episodic reward_yaw" :total_reward_yaw, "episodic reward_pr": total_reward_pr , "wrong action" : wrong_action}
            ...
            print(sum(ep_rews) , "in episode" ,episode_number, "up actions", up_actions, "down actions", down_actions ) # sum will add all the rewards
