...
fieldnames2 = ['epsodic reward' , 'total_positive_reward' , 'total_negative_reward' ,'closest_dist' ,'max_height' ,'end_distance' ,'max_distance' , 'rewarded_actions(100)' , 'norewarded_actions(0)', 'highest_x' , 'highest_y' , 'highest_pitch' , 'highest_roll' , 'epsodic reward_m' , 'episodic reward_d','episodic reward_xy' ,'episodic reward_pr' ]
m1 = 0
m2 = 0
m3 = 0
m4 = 0
...
global roll, pitch, yaw , lidar_distance ,velocity , global_x , global_y  , episode , total_reward_m, total_reward_d , total_reward_yaw, total_reward_xy , total_reward_pr , closest_dist , distance_goal , name_data , total_positive_reward , total_negative_reward  , end_distance , max_distance , episode_count , change 
...
class Environment():
    global m1 , m2 , m3 , m4 ,lidar_distance ,global_y , global_x ,roll, pitch, episode , total_reward_m, total_reward_d ,total_reward_xy , total_reward_yaw, total_reward_pr , closest_dist , distance_goal , name_data , total_positive_reward , total_negative_reward , end_distance , max_distance
    def __init__(self):
    ...
    def get_reward(self ,state, action):        
        ...
        #print("distance_goal",distance_goal , drone_position ) 
        ...
        #1.STABILITY 
        #1.1 PITCH ROLL ERROR    
        
        
        #reward_pr = -(abs(pitch)) + (-abs(roll))
        #print(-abs(pitch), -abs(roll))
        
        
        #1.2 YAW ERROR
        reward_yaw = -(abs((abs(yaw)-abs(state[14]))))
        #print(reward_yaw) #~0.4 max
        ...
        if (lidar_distance < 5) and abs(pitch) < 0.1 and abs(roll) < 0.1:
            reward_d = math.exp(-0.33723073*(distance_goal**2))
            #print("_d", math.exp(-0.33723073*(distance_goal**2)))
        elif (lidar_distance > 5):
            reward_d = ((lidar_distance-6))
            #print ("_d",-(lidar_distance-6))
        #LIDAR DISTANCE
...
class PPO():
    ...
    def rollout(self):
        global m1 , m2 , m3 , m4 ,lidar_distance ,global_y , global_x ,roll, pitch, episode , total_reward_m, total_reward_d ,total_reward_xy , total_reward_pr , closest_dist , distance_goal , name_data , total_positive_reward , total_negative_reward , end_distance , max_distance  ,highest_point ,highest_x , highest_y , highest_pitch , highest_roll , rewarded_actions , noreward_actions ,episode_count 
        batch_obs = []
        batch_acts = []
        batch_log_probs = []
        batch_rews = []
        ...
        while t < self.timesteps_per_batch:
            ...
            with open(name,'a') as csv_file_2:
                csv_writer_2 = csv.DictWriter(csv_file_2 , fieldnames = fieldnames2)
                information2 = {"epsodic reward" : sum(ep_rews) ,"total_positive_reward" : total_positive_reward, "total_negative_reward" : total_negative_reward, "closest_dist" : closest_dist, "max_height" : highest_point,"end_distance": end_distance,"max_distance": max_distance, "rewarded_actions(100)":rewarded_actions , "norewarded_actions(0)":norewarded_actions, "highest_x" : highest_x, "highest_y" : highest_y , "highest_pitch" : highest_pitch, "highest_roll" : highest_roll,"epsodic reward_m" : total_reward_m, "episodic reward_d" : total_reward_d, "episodic reward_xy" :total_reward_xy, "episodic reward_pr": total_reward_pr}
                #'epsodic reward' , 'total_positive_reward' , 'total_negative_reward' ,'closest_dist' ,'max_height' ,'end_distance' ,'max_distance' , 'rewarded_actions(100)' , 'norewarded_actions(0)', 'highest_x' , 'highest_y' , 'highest_pitch' , 'highest_roll' , 'epsodic reward_m' , 'episodic reward_d','episodic reward_yaw' ,'episodic reward_pr'
...
def service():
    global roll, pitch, yaw , lidar_distance , m1 , m2 , m3 , m4 ,episode , total_reward_m, total_reward_d ,total_reward_xy , total_reward_yaw, total_reward_pr , closest_dist , distance_goal , name_data , total_positive_reward , total_negative_reward , end_distance , max_distance ,highest_point ,highest_x , highest_y , highest_pitch , highest_roll , rewarded_actions , norewarded_actions 
