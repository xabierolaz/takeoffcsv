...
fieldnames = ['reward','reward_m','reward_d','reward_x','reward_y','reward_pr' , 'motor values','distance' , 'lidar_value' , 'x_coordinate' , 'y_coordiante' , 'pitch' ,'roll']
...
global roll, pitch, yaw , lidar_distance ,velocity , global_x , global_y  , episode , total_reward_m, total_reward_d ,total_reward_xy , total_reward_pr , closest_dist , distance_goal , name_data , total_positive_reward , total_negative_reward  , end_distance , max_distance , episode_count 
...
class Environment():
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(np.array([0.0,0.0,0.0,0.0]),np.array([1.0,1.0,1.0,1.0]),dtype=np.float32)
    ...
    def step(self,action,state):
        ...
        velocity.data = [abs(action[0]) ,-abs(action[0]) , abs(action[0]), -abs(action[0])]
    ...
    def get_reward(self , action):        
        ...
        with open(name_data + '.csv','a') as csv_file:
            ...
            information = { "reward" : reward , "reward_d" : reward_d , "distance" : distance_goal, "lidar_value" : lidar_distance,"reward_x" : reward_x,"reward_y" : reward_y ,"reward_pr" : reward_pr  ,"reward_m" : reward_m , "motor values" : [m1,m2,m3,m4], "x_coordinate" : global_x , "y_coordiante": global_y , "pitch" : pitch ,"roll" : roll }
