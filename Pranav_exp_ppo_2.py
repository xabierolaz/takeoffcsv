...
class Environment():
    ...
    def step(self,action,state ,w_a):
        ...
        pr_stable = False
        pr_unstable = False
        pr_out_of_control = False
        
        #PITCHROLL MED ERROR
        if (abs(pitch) <= pr_med_error and abs(roll) <= pr_med_error):
            pr_stable = True  
        elif (abs(pitch) > pr_max_error and  abs(roll) > pr_max_error):
            pr_out_of_control = True
        else:
            pr_unstable = True
        ...
        #IF WE GET AWAY FROM X OR Y ITS OK TO USE PITCH AND ROLL (TO A MAXIMUM) IF THE DISTANCE TO THE POINT IS BEING REDUCED

        

        if abs(global_x) > abs(global_y):
            if global_x > 1 :
                action = 5
                print("it should take action 4")
            elif global_x < -1 :
                action = 4
                print("it should take action 5")
            elif global_y >1 :
                action = 3
                print("it should take action 3")
            elif global_y <-1:
                action = 2
                print("it should take action 2")
        ...
        '''
        if global_x > 1 and pitch >-0.05:
            action = 4
            print("global_x > 1 action 4", global_x, action)  
         
        if pitch > 0.05 :  # one amonge this is  getting true so action is overwritten by that action got it?
            print("taking action 4")
            action = 4 #pitch -
        elif pitch < -0.05:
            print("taking action 5")
            action = 5  #pitch +
        elif roll < -0.05:
            print("against action", action)
            action = 3 #roll +
        elif roll > 0.05 :
        ...
        if lidar_distance > 5 :
            print("taking action 1 1st")
            action = 1  #down
      
           
        elif lidar_distance >=4 and lidar_distance <= 5:
            if (abs(m1) and abs(m2) and abs(m3) and abs(m4)) > 50: # when to execute these?
                action = 1
                print("taking action 1 2nd")
                #print("SMALL CORRECTION DOWN")
            elif (abs(m1) and abs(m2) and abs(m3) and abs(m4)) < 50:
                action = 0
                print("taking action 0 1st")
                #print("SMALL CORRECTION UP")
        
        elif lidar_distance < 4 :
            print("taking action 0 2nd")
            action = 0  #up 
        
        '''

        print(action , "actual action taken")
        ...
        '''
            Attempt 1 on 1st maymorning: 
            elif pr_stable == True: # added this let me know if correct
                print("if pr_stable")
                if global_x > 1 :
                    action = 4
                elif global_x < -1 :
            ...
            elif pr_unstable == True:
                print("if pr_unstable")
                if pitch < -0.05:
                    action = 5  #pitch frontwards(+)
                elif pitch > 0.05:
                    action = 4  #pitch bacwards(-)
                elif roll < -0.05:
                    action = 3  #roll right(+)
                elif roll > 0.05:
                    action = 2  #roll left(-) # this part right?
                    
            '''


        '''
            if (abs(global_x) < 1) and (abs(global_y)<1):  
       ...
       1- check for stability 
            ...
            stability range = -0.05 to 0.05
            max range = -0.1 to 0.1
        ...
        if its ok, check for position
            I only will make with one example but should be with 4, pitch +-, roll +-
            if global_x > 1 we are far away in the global x, so more than 1 meter ahead we want to get back to 1 meter
               if pitch>-0.1
               action = 4 #pitch-
               else:
                break
               repeat until if pitch < 0.1 if we get to the max limit # we can take only one action right?
                go to 1- check stability
        ...
        if abs(global_x) > 1 or abs(global_y) > 1:
            #position
            if global_x > 1:
                action = 4   #pitch -
                print("X>1, CORRECTION" , global_x)
            elif global_x < -1:
                action = 5  #pitch +
                print("X<-1, CORRECTION" , global_x)
            elif global_y > 1:
                action = 3   #roll +
                print("y>1, CORRECTION" , global_y)
            elif global_y < -1:
                action = 2  #roll -
                print("y<-1, CORRECTION" , global_y) # these are ok
            ...
            #stability
                ...
                action = 2  #roll -
            #height
            elif lidar_distance < 4.5 :
                action = 0  #up
           
           #this the last thing, so 4
            elif (lidar_distance >= 4.5) and (lidar_distance <= 5):
                if (abs(m1) and abs(m2) and abs(m3) and abs(m4)) > 50: # when to execute these?
                    action = 1
                    #print("SMALL CORRECTION DOWN")
                elif (abs(m1) and abs(m2) and abs(m3) and abs(m4)) < 50:
        ...
        if action == 0 : #up
            ...
            #print("action 2")
            m1 = state[18] - change/10
            m2 = state[19] + change/10
            m3 = state[20] + change/10
            m4 = state[21] - change/10
        elif action == 3 : #roll +
            #print("action 3")
            m1 = state[18] + change/10
            m2 = state[19] - change/10
    ...
    def get_reward(self , state, action , wrong_action):        
        ...
        THIS NOT YET
        if lidar_distance < 5:
            reward_d = (5 - lidar_distance) * 0.01
            print("lidar",lidar_distance, "goal",distance_goal)
        elif lidar_distance > 5:
            reward_m = (7 - lidar_distance) * 0.01 
        else:
            reward_d = 0
            reward_m = 0  
        ...
        if distance_goal < 5:
            if  distance_goal < state[17]:
                reward_d = 5-distance_goal
            
        '''
        if action == 0:
            reward_d = distance_goal
        '''   
        ...
        if  distance_goal < state[17]:
            reward_d = distance_goal
            print ("distance_goal", distance_goal, "state[17]", state[17])
        ...
        if ((m1 and m3) = 100) and ((m2 and m4) >= 100):
            print (m1,m2,m3,m4)

        
        if (global_x <2 and global_y < 1 and lidar_distance < 6):
            if ((lidar_distance > 0) and (lidar_distance < 4)) and (state[13] < state[2]) :
                reward_d = (lidar_distance)*0.1
            elif ((lidar_distance > 4) and (lidar_distance < 5)) and (abs(state[11]) <= abs(state[0])) :
                reward_m = (lidar_distance)*0.3
            elif (lidar_distance > 5) and ((abs(state[0]) - abs(state[11]) >= 0.0001 )):
                reward_pr = distance_goal*0.5
        ...
        if ((lidar_distance > 0) and (lidar_distance < 4)) and action == 0 :
            reward_d = 0.1
            up_actions += 1
        elif (lidar_distance > 5) and (action == 1):
            reward_m = 0.1
            down_actions += 1
        else:
            wrong_action += 1
          ...
          #state version
        if (abs(global_x) < 2 and abs(global_y) < 2):               
            if (pitch > -0.05):
                if action == 2:
                    reward_d = 0.02
                else:
                    reward_d = 0
                    wrong_action = wrong_action + 1
            elif (pitch > 0.05):
                if action == 3:
                    reward_d = 0.02
                else:
