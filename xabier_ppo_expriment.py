...
class agent(object):
    def __init__( self , batch_size  , n_epochs , n_actions = 9   , input_dims = 44):
        ...
        #self.actor.apply(self.weights_init_uniform_rule)
        ...
        #self.critic.apply(self.weights_init_uniform_rule)
...
class Environment():
    ...
    def get_reward(self , state , action):        
        ...
        if distance_goal < 6 and lidar_distance < 8 :
            reward_d = (math.exp(-0.16634528*(distance_goal**2)))*0.05
            total_positive_reward += reward_d
        else:
            reward_d = 0
        '''
        #PRANAV
        ...
        if distance_goal < 6 and lidar_distance < 8:
            reward_d = (math.exp(-0.16634528*(distance_goal**2)))
        else:
            reward_d = 0

        print(distance_goal)
        '''
        if state[4] > 6 and state[4] < 7 :
            reward_d = 1
        ...
        if state[4] > 7 :
            ...
            reward_d = -(distance_goal * 0.000001) 
            total_negative_reward += reward_d

        
        elif lidar_distance > 8:
            reward_d = -(lidar_distance * 0.001)
            total_negative_reward += reward_d 
            #add it to negative
            #add it to negative
        '''
        ...
        #total reward_d = positive + negative
        '''
        #extra adjustment
...
def service():
    global roll, pitch, yaw , lidar_distance , m1 , m2 , m3 , m4 ,episode , total_reward_m, total_reward_d ,total_reward_y , total_reward_rp , closest_dist , distance_goal , name_data , total_positive_reward , total_negative_reward , end_distance , max_distance
    env = Environment()
    N =2048
    batch_size = 128
    n_epochs = 4
    ...
    for i in range(Number_of_episodes):
        ...
        episode = i
        name_data = my_folder + str(episode) 
        ...
        if ep_reward >= np.mean(reward_history):
            T.save(drone.actor,ac_weights_folder +str(i)+'.pth')
            T.save(drone.actor,cr_weights_folder +str(i)+'.pth')
           

    T.save(drone.actor, ac_weights_folder +'actor_weights_final.pth')
    T.save(drone.actor, cr_weights_folder +'critic_weights_final.pth')
